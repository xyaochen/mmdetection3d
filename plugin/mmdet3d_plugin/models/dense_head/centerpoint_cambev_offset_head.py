...
def feature_sampling(mlvl_feats, reference_points,  pc_range, img_metas):
    ...
    reference_points = reference_points.view(B, 1, num_query*num_points, 4).repeat(1, num_cam, 1, 1).unsqueeze(-1)
    # lidar2img chaneg to (B, num_cam, num_query, 4, 4)
    lidar2img = lidar2img.view(B, num_cam, 1, 4, 4).repeat(1, 1, num_query*num_points, 1, 1)
    ...
    for lvl, feat in enumerate(mlvl_feats):
        ...
        reference_points_cam_lvl = reference_points_cam.view(B*N, num_query, num_points, 2).to(feat.device)
...
class BEVCenterHeadOffset(BaseModule):
    ...
    def __init__(self,
        ...
        super(BEVCenterHeadOffset, self).__init__(init_cfg=init_cfg)
        ...
        self.BEV_query = nn.Embedding(self.x_size*self.y_size, self.embed_dims)
        self.offsets = nn.Linear(embed_dims, z_size*num_points*3)
        ...
        self.attention_weights = nn.Linear(embed_dims, z_size*num_cams*num_levels*num_points)
    ...
    def forward(self, feats, img_metas):
        ...
        reference_points = reference_points.to(feats[0].device)
        offsets = self.offsets(BEV_query).view(batch_size, self.x_size*self.y_size*self.z_size, self.num_points, 3)
        # [-0.5, 0.5]
        offsets = offsets.sigmoid() - 0.5
        reference_points = reference_points.view(batch_size, -1, 1, 3) + offsets
        ...
        PE_embed = self.position_enc(reference_points).view(batch_size, self.embed_dims, self.y_size, self.x_size, self.z_size*self.num_points)
        PE_embed = PE_embed.sum(-1)

        num_query = self.x_size * self.y_size
        attention_weights = self.attention_weights(BEV_query).view(batch_size, 1, num_query, self.z_size, self.num_cams, self.num_points, self.num_levels)
        
        #reference_points = reference_points.view(batch_size, -1, 3)
        # sampled_feats (B, C, num_query, num_cam, num_points, len(lvl_feats))
        BEV_feats = feature_sampling(feats, reference_points, self.pc_range, img_metas)
        BEV_feats = BEV_feats.view(batch_size, self.embed_dims, num_query, self.z_size, self.num_cams, self.num_points, self.num_levels)
        BEV_feats = BEV_feats * attention_weights.sigmoid()
        ...
        return multi_apply(self.forward_single, BEV_feats)
