...
def feature_sampling_2D(bev_feat, reference_points, offsets, bev_size):
    ...
    B, num_query, num_points = offsets.size()[:3]
    ...
    reference_points_rel = reference_points_rel.view(B, num_query, 1, 2) + offsets
    ...
    reference_points_rel = reference_points_rel.view(B, num_query, num_points, 2)
    sampled_feat = F.grid_sample(bev_feat, reference_points_rel)
    sampled_feat = sampled_feat.view(B, C, num_query, num_points)
...
class LocalTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", num_points=8,
        ...
        self.offsets = nn.Linear(d_model, num_points*2)
    ...
    def forward(self, query, key, query_pos, key_pos, bev_feat, bev_size, attn_mask=None):
        ...
        '''
        offsets = self.offsets(query).view(B, N, self.num_points, 2)
        ...
        sampled_feat = sampled_feat * offsets_weights.sigmoid()
        sampled_feat = sampled_feat.sum(-1).permute(2, 0, 1)
        query = query + self.dropout2(sampled_feat)
...
class TransFusionLocalHead(nn.Module):
    def __init__(self,
                 fuse_img=False,
                 num_views=0,
                 in_channels_img=64,
        ...
        super(TransFusionLocalHead, self).__init__()
