...
class DeformableFUTR3DHeadLiDAR(DETRHead):
    ...
    def __init__(self,
        ...
        super(DeformableFUTR3DHeadLiDAR, self).__init__(
    ...
    def forward(self, mlvl_pts_feats, img_metas):
        ...
        # shape [num_dec_layers, bs, num_query, num_classes]
        outputs_classes = torch.stack(outputs_classes)
        # shape [num_dec_layers, bs, num_query, bbox_size]
    ...
    def _get_target_single(self,
        ...
        # PseudoSampler does not do sampling actually, directly returns the 
        # positive and negative indices  of samples
    ...
    def loss_single(self,
        ...
        Args:
            cls_scores (Tensor): Box score logits from a single decoder layer
                for all samples. Shape [bs, num_query, cls_out_channels].
            bbox_preds (Tensor): Sigmoid outputs from a single decoder layer
                for all samples, with normalized coordinate (cx, cy, w, h) and
    ...
    def loss(self,
        ...
        Args:
            ...
            gt_bboxes_list (list[Tensor]): Ground truth bboxes for each sample
                with shape (num_gts, 4) in [tl_x, tl_y, br_x, br_y] format.
            gt_labels_list (list[Tensor]): Ground truth class indices for each
                image with shape (num_gts, ).
            preds_dicts:
                ...
                all_bbox_preds (Tensor): Sigmoid regression
                    outputs of all decode layers. Each is a tensor with
                    normalized coordinate format (cx, cy, w, l, cz, h, rot_sin, rot_cos) 
                    and shape [nb_dec, bs, num_query, 8] for kitti.
