...
from .transfusion_head import TransFusionHead, MultiheadAttention, PositionEmbeddingLearned, FFN

def feature_sampling_2D(bev_feat, reference_points, offsets, bev_size):
    reference_points = reference_points.clone()
    reference_points_rel = reference_points[..., 0:2]
    #print('ref_point_rel', reference_points_rel.size())
    # reference_points (B, num_heads, num_queries, num_points, 2)
    B, num_heads, num_query, num_points = offsets.size()[:4]

    reference_points_rel[..., 0] = reference_points[..., 0] / bev_size[0]
    reference_points_rel[..., 1] = reference_points[..., 1] / bev_size[1]
    
    offsets = offsets.sigmoid()
    reference_points_rel = reference_points_rel.view(B, 1, num_query, 1, 2) + offsets
    reference_points_rel = reference_points_rel.clamp(min=1e-5, max=1-1e-5)
    reference_points_rel = (reference_points_rel - 0.5) * 2
    #print(reference_points_rel.size())
    #for lvl, feat in enumerate(mlvl_feats):
    B, C, H, W = bev_feat.size()
    
    reference_points_rel = reference_points_rel.view(B*num_heads, num_query, num_points, 2)
    sampled_feat = F.grid_sample(bev_feat.view(B*num_heads, C // num_heads, H, W), reference_points_rel)
    sampled_feat = sampled_feat.view(B, num_heads, C // num_heads, num_query, num_points)
    
    return sampled_feat


class LocalTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", num_points=16,
                 num_outs=64, self_posembed=None, cross_posembed=None, cross_only=False):
        super().__init__()
        ...
        self.offsets = nn.Linear(d_model, nhead*num_points*2)
        #self.offsets_weights = nn.Linear(d_model, num_points)
        #self.cross_posembed = cross_posembed
        self.group_dim = d_model // nhead
        self.channel_mix = self.group_dim * self.group_dim
        self.spatial_mix = num_points * num_outs
        self.parameter_generator = nn.Linear(d_model, nhead*(self.channel_mix + self.spatial_mix))

        self.out_proj = nn.Linear(d_model*num_outs, d_model, bias=True)

        self.channel_norm = nn.LayerNorm([self.num_points, self.group_dim])
        self.spatial_norm = nn.LayerNorm([self.num_outs, self.group_dim])
        self.act = nn.ReLU(inplace=True)

    def with_pos_embed(self, tensor, pos_embed):
        return tensor if pos_embed is None else tensor + pos_embed

    def forward(self, query, key, query_pos, key_pos, bev_feat, bev_size, attn_mask=None):
        """
        :param query: B C Pq
        :param key: B C Pk
        ...
        '''
        para = self.parameter_generator(query)
        channel_para, spatial_para = para.split([self.n_head*self.channel_mix, self.n_head*self.spatial_mix], dim=2)

        offsets = self.offsets(query).view(B, self.n_head, N, self.num_points, 2)
        #offsets_weights = self.offsets_weights(query).view(B, 1, N, self.num_points)
        # shape [B, num_heads, C//num_heads, num_query, num_points]
        sampled_feat = feature_sampling_2D(bev_feat, query_pos, offsets, bev_size)
        sampled_feat = sampled_feat.view(B*N*self.n_head, self.num_points, self.group_dim)

        channel_para = channel_para.reshape(B*N*self.n_head, self.group_dim, self.group_dim)
        mixed_feat = torch.bmm(sampled_feat, channel_para)
        mixed_feat = self.channel_norm(mixed_feat)
        mixed_feat = self.act(mixed_feat)

        spatial_para = spatial_para.reshape(B*N*self.n_head, self.num_outs, self.num_points)
        mixed_feat = torch.bmm(spatial_para, mixed_feat)
        mixed_feat = self.spatial_norm(mixed_feat)
        mixed_feat = self.act(mixed_feat)

        mixed_feat = self.out_proj(mixed_feat.view(N, B, -1))
        
        #PosEmbed = self.with_pos_embed()

        query = query + self.dropout2(mixed_feat)
...
class TransFusionLocalMixHead(nn.Module):
    def __init__(self,
                 fuse_img=False,
                 num_views=0,
                 in_channels_img=64,
        ...
        super(TransFusionLocalMixHead, self).__init__()
    ...
    def forward_single(self, inputs, img_inputs, img_metas):
        ...
        for i in range(self.num_decoder_layers):
            ...
            query_feat = self.decoder[i](query_feat, lidar_feat_flatten, query_pos, bev_pos, lidar_feat, self.bev_size)
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d, preds_dict, batch_idx):
        ...
        for idx_layer in range(num_layer):
            ...
            if self.train_cfg.assigner.type == 'TFHungarianAssigner3D':
