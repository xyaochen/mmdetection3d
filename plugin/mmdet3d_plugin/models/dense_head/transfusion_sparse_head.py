...
def feature_sampling_2D(bev_feat, reference_points, bev_size):
    ...
    reference_points_rel = reference_points_rel.view(B, int(num_query/10), 10, 2)
    sampled_feat = F.grid_sample(bev_feat, reference_points_rel)
    sampled_feat = sampled_feat.view(B, C, num_query)
...
class SparseTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, query, key, query_pos, key_pos, bev_feat, bev_size, attn_mask=None):
        ...
        '''
        sampled_feat = feature_sampling_2D(bev_feat, query_pos, bev_size).permute(2, 0, 1)
...
class TransFusionSparseHead(nn.Module):
    def __init__(self,
                 fuse_img=False,
                 num_views=0,
                 in_channels_img=64,
        ...
        super(TransFusionSparseHead, self).__init__()
