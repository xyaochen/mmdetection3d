...
from mmcv.cnn import Conv2d, Linear, bias_init_with_prob, constant_init,  build_conv_layer
...
class FUTR3DDETRHeadLiDAR(DETRHead):
    ...
    def __init__(self,
        ...
        super(FUTR3DDETRHeadLiDAR, self).__init__(
        ...
        self.bev_pos_enc = nn.Sequential(
    ...
    def _init_layers(self):
        ...
        center_reg_branch.append(Linear(self.embed_dims, 3))
    ...
    def forward(self, mlvl_pts_feats, img_metas):
        ...
        pts_feats = mlvl_pts_feats[0].squeeze(dim=1)
        #print("pts_feats", pts_feats.size())
        pts_feats = self.input_proj(pts_feats)
        #print("pts_feats", pts_feats.size())
        #print(mlvl_pts_feats.size())
        # shape [B, C, H*W]
        pts_feats_flatten = pts_feats.view(batch_size, pts_feats.shape[1], -1)
        #print("pts_feats_flatten", pts_feats_flatten.size())
        query_embeds = self.query_embedding.weight
        bev_pos = self.bev_pos.repeat(batch_size, 1, 1).to(pts_feats_flatten.device)
        #print('bev_pos', bev_pos.size())
        pts_pos = self.bev_pos_enc(bev_pos).permute(0, 2, 1)
