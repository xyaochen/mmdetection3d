...
@ATTENTION.register_module()
class MultiheadAttentionLiDAR(BaseModule):
    """A wrapper for ``torch.nn.MultiheadAttention``.
    This module implements MultiheadAttention with identity connection,
    and positional encoding  is also passed as input.
    Args:
    ...
    def __init__(self,
                 ...
                 batch_first=False,
                 **kwargs):
        super(MultiheadAttentionLiDAR, self).__init__(init_cfg)
        if 'dropout' in kwargs:
            warnings.warn('The arguments `dropout` in MultiheadAttention '
                          'has been deprecated, now you can separately '
                          'set `attn_drop`(float), proj_drop(float), '
    ...
    def forward(self,
        ...
        **kwargs allow passing a more general data flow when combining
        ...
        if key_pos is not None:
            key = key + key_pos
        #print("in dense cross attention")
        out = self.attn(
            query=query,
            key=key,
            value=value,
...
@ATTENTION.register_module()
class FUTR3DCrossAttenCamLiDAR(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(FUTR3DCrossAttenCamLiDAR, self).__init__(init_cfg)
...
@ATTENTION.register_module()
class FUTR3DCrossAttenCam(BaseModule):
    """An attention module used in Deformable-Detr. `Deformable DETR:
    Deformable Transformers for End-to-End Object Detection.
      <https://arxiv.org/pdf/2010.04159.pdf>`_.
    Args:
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(FUTR3DCrossAttenCam, self).__init__(init_cfg)
        if embed_dims % num_heads != 0:
            raise ValueError(f'embed_dims must be divisible by num_heads, '
                             f'but got {embed_dims} and {num_heads}')
        dim_per_head = embed_dims // num_heads
    ...
    def forward(self,
        ...
        # (num_query, bs, embed_dims)
        return self.dropout(img_output) + inp_residual + self.pos_encoder(inverse_sigmoid(reference_points_3d)).permute(1, 0, 2)



@ATTENTION.register_module()
class FUTR3DCrossAttenLiDAR(BaseModule):
    """An attention module used in Deformable-Detr. `Deformable DETR:
    Deformable Transformers for End-to-End Object Detection.
      <https://arxiv.org/pdf/2010.04159.pdf>`_.
    Args:
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(FUTR3DCrossAttenLiDAR, self).__init__(init_cfg)
        if embed_dims % num_heads != 0:
            raise ValueError(f'embed_dims must be divisible by num_heads, '
                             f'but got {embed_dims} and {num_heads}')
        dim_per_head = embed_dims // num_heads
...
@ATTENTION.register_module()
class FUTR3DCrossAttenLiDARWPos(BaseModule):
    """An attention module used in Deformable-Detr. `Deformable DETR:
    Deformable Transformers for End-to-End Object Detection.
      <https://arxiv.org/pdf/2010.04159.pdf>`_.
    Args:
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(FUTR3DCrossAttenLiDARWPos, self).__init__(init_cfg)
        if embed_dims % num_heads != 0:
            raise ValueError(f'embed_dims must be divisible by num_heads, '
                             f'but got {embed_dims} and {num_heads}')
        dim_per_head = embed_dims // num_heads
    ...
    def forward(self,
        ...
        pts_output = pts_output.sum(-1).sum(-1).sum(-1)
        pts_output = pts_output.permute(2, 0, 1)
        print('pts_output', pts_output.size())
        pts_output = self.pts_output_proj(pts_output)

        # reference_points_3d = reference_points.clone()
        # (num_query, bs, embed_dims)
        return self.dropout(pts_output) + inp_residual



@ATTENTION.register_module()
class FUTR3DCrossAttenCamLiDARV2(BaseModule):
    """An attention module used in Deformable-Detr. `Deformable DETR:
    Deformable Transformers for End-to-End Object Detection.
      <https://arxiv.org/pdf/2010.04159.pdf>`_.
    Args:
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(FUTR3DCrossAttenCamLiDARV2, self).__init__(init_cfg)
        if embed_dims % num_heads != 0:
            raise ValueError(f'embed_dims must be divisible by num_heads, '
                             f'but got {embed_dims} and {num_heads}')
        dim_per_head = embed_dims // num_heads
        ...
        self.img_output_proj = nn.Linear(embed_dims, embed_dims)
        self.pts_output_proj = nn.Linear(embed_dims, embed_dims)
        self.cross_modality_attn = nn.MultiheadAttention(2*embed_dims, num_heads=8)
        self.modality_fusion_layer = nn.Sequential(
            nn.Linear(2*self.embed_dims, self.embed_dims),
            nn.LayerNorm(self.embed_dims),
            nn.ReLU(inplace=True),
    ...
    def forward(self,
        ...
        img_output = img_output * attention_weights
        # output (B, emb_dims, num_query, num_points, len(lvl_feats))
        img_output = img_output.sum(-1).sum(-1).sum(-1)
        img_output = img_output.permute(2, 0, 1)
        img_output = self.img_output_proj(img_output)
        ...
        pts_attention_weights = pts_attention_weights.sigmoid()
        pts_output = pts_output * pts_attention_weights
        pts_output = pts_output.sum(-1).sum(-1).sum(-1)
        pts_output = pts_output.permute(2, 0, 1)
        pts_output = self.pts_output_proj(pts_output)
        
        mul_output = torch.cat((img_output, pts_output), -1)
        #mul_output = mul_output.permute(2, 0, 1)
        #print('mul_output', mul_output.size())
        output, _ = self.cross_modality_attn(mul_output, mul_output, mul_output)
        
        output = output + self.dropout(mul_output)
        #output = mul_output.sum(0).view(bs, num_query, self.embed_dims).permute(1, 0, 2)
...
@ATTENTION.register_module()
class FUTR3DCrossAttenCamLiDARSENet(BaseModule):
    """An attention module used in Deformable-Detr. `Deformable DETR:
    Deformable Transformers for End-to-End Object Detection.
      <https://arxiv.org/pdf/2010.04159.pdf>`_.
    Args:
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(FUTR3DCrossAttenCamLiDARSENet, self).__init__(init_cfg)
        if embed_dims % num_heads != 0:
            raise ValueError(f'embed_dims must be divisible by num_heads, '
                             f'but got {embed_dims} and {num_heads}')
        dim_per_head = embed_dims // num_heads
        ...
        self.SE = nn.Sequential(
            nn.Linear(2*self.embed_dims, 2*self.embed_dims // reduction, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(2*self.embed_dims // reduction, 2*self.embed_dims, bias=False),
            nn.Sigmoid()
        )
        self.modality_fusion_layer = nn.Sequential(
            nn.Linear(2*self.embed_dims, self.embed_dims),
    ...
    def forward(self,
        ...
        #mul_output = mul_output.permute(2, 0, 1)
        #print('mul_output', mul_output.size())
        # shape [B, 2*self.embeds]
        se_act = self.avg_pool(mul_output.permute(1, 2, 0)).squeeze()
        se_act = self.SE(se_act).view(1, bs, -1)
        output = mul_output * se_act
...
@ATTENTION.register_module()
class FUTR3DCrossAttenCamLiDARSENetV2(BaseModule):
    """An attention module used in Deformable-Detr. `Deformable DETR:
    Deformable Transformers for End-to-End Object Detection.
      <https://arxiv.org/pdf/2010.04159.pdf>`_.
    Args:
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(FUTR3DCrossAttenCamLiDARSENetV2, self).__init__(init_cfg)
        if embed_dims % num_heads != 0:
            raise ValueError(f'embed_dims must be divisible by num_heads, '
                             f'but got {embed_dims} and {num_heads}')
        dim_per_head = embed_dims // num_heads
        ...
        self.SE = nn.Sequential(
            nn.Linear(2*self.embed_dims, 2*self.embed_dims // reduction, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(2*self.embed_dims // reduction, self.embed_dims, bias=False),
            nn.Sigmoid()
        )
        self.modality_fusion_layer = nn.Sequential(
            nn.Linear(2*self.embed_dims, self.embed_dims),
    ...
    def forward(self,
        ...
        #print(img_output.size(), pts_output.size())
        #output = torch.cat((img_output, pts_output), dim=2).permute(1, 0, 2)
        output = self.modality_fusion_layer(mul_output)
        output = output * se_act
        # (num_query, bs, embed_dims)
        return self.dropout(output) + inp_residual + self.pos_encoder(inverse_sigmoid(reference_points_3d)).permute(1, 0, 2)
...
@ATTENTION.register_module()
class FUTR3DCrossAttenCamLiDARSENetV3(BaseModule):
    """An attention module used in Deformable-Detr. `Deformable DETR:
    Deformable Transformers for End-to-End Object Detection.
      <https://arxiv.org/pdf/2010.04159.pdf>`_.
    Args:
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(FUTR3DCrossAttenCamLiDARSENetV3, self).__init__(init_cfg)
        if embed_dims % num_heads != 0:
            raise ValueError(f'embed_dims must be divisible by num_heads, '
                             f'but got {embed_dims} and {num_heads}')
        dim_per_head = embed_dims // num_heads
        ...
        self.pts_output_proj = nn.Linear(embed_dims, embed_dims)
        self.avg_pool = nn.AdaptiveAvgPool1d(1)
        self.SE_dim_weights = nn.Linear(self.embed_dims, 2*self.embed_dims)
        self.SE = nn.Sequential(
            nn.Linear(2*self.embed_dims, 2*self.embed_dims // reduction, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(2*self.embed_dims // reduction, self.embed_dims, bias=False),
    ...
    def forward(self,
        ...
        # shape [B, 2*self.embeds]
        se_act = self.avg_pool(mul_output.permute(1, 2, 0)).squeeze()
        query_act = self.avg_pool(query.permute(0, 2, 1)).squeeze()
        query_act = self.SE_dim_weights(query_act)
        se_act = se_act * query_act
        se_act = self.SE(se_act).view(1, bs, -1)
...
@ATTENTION.register_module()
class FUTR3DCrossAttenCamLiDARSENetV4(BaseModule):
    """An attention module used in Deformable-Detr. `Deformable DETR:
    Deformable Transformers for End-to-End Object Detection.
      <https://arxiv.org/pdf/2010.04159.pdf>`_.
    Args:
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(FUTR3DCrossAttenCamLiDARSENetV4, self).__init__(init_cfg)
        if embed_dims % num_heads != 0:
            raise ValueError(f'embed_dims must be divisible by num_heads, '
                             f'but got {embed_dims} and {num_heads}')
        dim_per_head = embed_dims // num_heads
        ...
        self.pts_output_proj = nn.Linear(embed_dims, embed_dims)
        self.avg_pool = nn.AdaptiveAvgPool1d(1)
        self.SE_dim_weights = nn.Linear(self.embed_dims, 2*self.embed_dims*self.embed_dims)
        self.SE = nn.Sequential(
            nn.Linear(self.embed_dims, self.embed_dims // reduction, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(self.embed_dims // reduction, self.embed_dims, bias=False),
            nn.Sigmoid()
        )
        self.modality_fusion_layer = nn.Sequential(
            nn.Linear(self.embed_dims, self.embed_dims),
    ...
    def forward(self,
        ...
        #print('mul_output', mul_output.size())
        # shape [B, 2*self.embeds]
        query_act = self.SE_dim_weights(query).view(bs*num_query, 2*self.embed_dims, self.embed_dims)
        mul_output = mul_output.view(bs*num_query, 1, 2*self.embed_dims)
        mul_output = torch.bmm(mul_output, query_act)
        mul_output = mul_output.view(num_query, bs, self.embed_dims)
...
@ATTENTION.register_module()
class FUTR3DCrossAttenLiDARDII(BaseModule):
    """An attention module used in Deformable-Detr. `Deformable DETR:
    Deformable Transformers for End-to-End Object Detection.
      <https://arxiv.org/pdf/2010.04159.pdf>`_.
    Args:
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(FUTR3DCrossAttenLiDARDII, self).__init__(init_cfg)
        if embed_dims % num_heads != 0:
            raise ValueError(f'embed_dims must be divisible by num_heads, '
                             f'but got {embed_dims} and {num_heads}')
        dim_per_head = embed_dims // num_heads
        ...
        #self.pts_output_proj = nn.Linear(embed_dims, embed_dims)
        self.interact = DynamicConv(embed_dim=self.embed_dims, dim_dynamic=self.embed_dims // 4)
    ...
    def forward(self,
        ...
        pts_attention_weights = self.weight_dropout(pts_attention_weights.sigmoid())
        pts_output = pts_output * pts_attention_weights
        #pts_output = pts_output.view(bs, self.embed_dims, num_query, self.num_levels)
        pts_output = pts_output.view(self.num_levels, bs * num_query, self.embed_dims)
        query = query.view(1, bs*num_query, self.embed_dims)
        output = self.interact(query, pts_output)

        output = output.view(num_query, bs, self.embed_dims)
        #pts_output = self.pts_output_proj(pts_output)
...
class DynamicConv(nn.Module):
    ...
    def __init__(self, embed_dim, dim_dynamic, num_dynamic=2, num_lvl=4):
        ...
        num_output = self.hidden_dim * num_lvl
    ...
    def forward(self, pro_features, roi_features):
        ...
        sampled_features: (num_lvl, N * nr_boxes, self.d_model)
        '''
        ...
        features = self.activation(features)
        # return shape [N * num_query, embed_dims]
        return features
...
class DynamicConvV2(nn.Module):
    ...
    def __init__(self, embed_dim, dim_dynamic, num_dynamic=2, num_lvl=4):
        ...
        self.dynamic_layer = nn.Linear(self.embed_dim, self.num_dynamic * self.num_params)
    ...
    def forward(self, query, sampled_feats):
        '''
        pro_features: (bs, num_query, embed_dim)
        sampled_feats: (bs, num_query, embed_dim)
        '''
        features = sampled_feats.permute(1, 0, 2)
        parameters = self.dynamic_layer(query).permute(1, 0, 2)

        param1 = parameters[:, :, :self.num_params].view(-1, self.embed_dim, self.dim_dynamic)
        param2 = parameters[:, :, self.num_params:].view(-1, self.dim_dynamic, self.embed_dim)
        ...
        features = self.norm3(features)
        features = self.activation(features)
        # return shape [bs, num_query, embed_dims]
        return features

@ATTENTION.register_module()
class FUTR3DCrossAttenLiDARDIIV3(BaseModule):
    """An attention module used in Deformable-Detr. `Deformable DETR:
    Deformable Transformers for End-to-End Object Detection.
      <https://arxiv.org/pdf/2010.04159.pdf>`_.
    Args:
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(FUTR3DCrossAttenLiDARDIIV3, self).__init__(init_cfg)
        if embed_dims % num_heads != 0:
            raise ValueError(f'embed_dims must be divisible by num_heads, '
                             f'but got {embed_dims} and {num_heads}')
        dim_per_head = embed_dims // num_heads
        ...
        #self.pts_output_proj = nn.Linear(embed_dims, embed_dims)
        self.interact = DynamicConvV2(embed_dim=self.embed_dims, dim_dynamic=self.embed_dims // 4)
    ...
    def forward(self,
        ...
        pts_attention_weights = self.weight_dropout(pts_attention_weights.sigmoid())
        pts_output = pts_output * pts_attention_weights
        pts_output = pts_output.sum(-1).sum(-1).sum(-1).permute(0, 2, 1)
        #print('pts_output', pts_output.size())
        output = self.interact(query, pts_output)
