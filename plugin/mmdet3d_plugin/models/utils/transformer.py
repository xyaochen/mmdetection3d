...
class FUTR3DTransformerCamLiDAR(BaseModule):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(FUTR3DTransformerCamLiDAR, self).__init__(**kwargs)
    ...
    def forward(self,
        ...
        '''
        #mlvl_feats list[(B, N, C, H, W)] --> value (S, N, C)
        for i in range(len(mlvl_feats)):
            feat = mlvl_feats[i]
            B, N, C, H, W = feat.size()
            mlvl_feats[i] = feat.permute(0, 2, 1, 3, 4).reshape(B, C, N*H*W).permute(2, 0, 1)
        value = mlvl_feats[0]
        for i in range(1, len(mlvl_feats)):
            value = torch.cat((value, mlvl_feats[i]), 0)
        print(query.size())
        print(value.size())
        '''
...
class FUTR3DTransformerDecoderCamLiDAR(TransformerLayerSequence):
    ...
    def __init__(self, *args, return_intermediate=False,  **kwargs):
        ...
        super(FUTR3DTransformerDecoderCamLiDAR, self).__init__(*args, **kwargs)
...
# you may need to change other transformer functions
...
class Deformable3DDetrTransformer(Transformer):
    ...
    def forward(self,
        ...
        if self.as_two_stage:
            ...
            enc_outputs_coord_unact[..., :4] += output_proposals
            ...
            reference_points = topk_coords_unact.sigmoid()[..., :4]
            init_reference_out = reference_points
