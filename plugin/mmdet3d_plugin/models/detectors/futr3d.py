...
from plugin.mmdet3d_plugin.vis import save_bev

@DETECTORS.register_module()
class FUTR3DCamLiDAR(MVXTwoStageDetector):
    ...
    def forward_dummy(self, points):
        #print(points.size())
        imgs = torch.rand(1, 6, 3, 928, 1600).to(points.device)
        img_metas = [dict()]
        img_metas[0]['lidar2img'] = [np.zeros((4,4)) for i in range(6)]
        img_metas[0]['img_shape'] = [[900, 1600] for i in range(6)]
        img_feats, pts_feats = self.extract_feat(
...
@DETECTORS.register_module()
class FUTR3DLiDAR(MVXTwoStageDetector):
    """Base class of Multi-modality VoxelNet."""
    ...
    def extract_pts_feat(self, pts):
        ...
        #print(batch_size)
        x = self.pts_middle_encoder(voxel_features, coors, batch_size.item())
        #print('after middle encoder', x.size())
        x = self.pts_backbone(x)
        #for feat in x:
        #    print('after backbone', feat.size())
        if self.with_pts_neck:
            x = self.pts_neck(x)
        #for feat in x:
        #    print('after neck', feat.size())
        return x
    ...
    def forward_pts_train(self,
        ...
        Returns:
            dict: Losses of each branch.
        """
        #print('gt_bbox', len(gt_bboxes_3d), gt_bboxes_3d[0].tensor.size())
        outs = self.pts_bbox_head(pts_feats, img_metas)
        #save_bev(points, outs['ref'], 'bev')
        #save_bev(points, outs['init_ref'], 'init_bev')
        #outs.pop('ref')
        #outs.pop('init_ref')
        loss_inputs = [gt_bboxes_3d, gt_labels_3d, outs]
        losses = self.pts_bbox_head.loss(*loss_inputs)
    ...
    def forward_dummy(self, points):
        #print(points.size())
        B, N, C = points.size()
        pts = torch.rand(B, N, C).to(points.device)
        pts[..., :2] *= 50.4
        pints = pts
        img_feats, pts_feats = self.extract_feat(
            points, img=None, img_metas=None)
...
@DETECTORS.register_module()
class FUTR3DCam(MVXTwoStageDetector):
    """Base class of Multi-modality VoxelNet."""
