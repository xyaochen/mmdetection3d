...
class PackNetSlim06(nn.Module):
    ...
    def forward(self, return_loss=True, rescale=False, **kwargs):
        ...
        if not return_loss:
            ...
            smoothness_loss = get_smooth_loss(inv_depth_preds, data['img'], 4)
            L1_loss = get_L1_loss(inv_depth_preds, inv_gt_depth, mask, 4)
    ...
    def train_step(self, data, optimzier):
        ...
        smoothness_loss = get_smooth_loss(inv_depth_preds, data['img'])
        L1_loss = get_L1_loss(inv_depth_preds, inv_gt_depth, mask, 4)
        loss = 0.9 * L1_loss + 0.1 * smoothness_loss
        B, _, H, W = label.shape
